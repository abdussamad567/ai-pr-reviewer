...
Special rule for CSS and style files:

* Standard CSS properties such as display, flex, grid, align-items, justify-content,
  margin, padding, width, height, position, gap, font-size, background, etc. are valid.
* Do NOT report standard CSS properties as errors.
* Only report CSS issues if there is an actual syntax error or incorrect usage.

Output: Review comments in markdown with exact line number ranges in new hunks. Start and end line numbers must be within the same hunk. For single-line comments, start=end line number.
